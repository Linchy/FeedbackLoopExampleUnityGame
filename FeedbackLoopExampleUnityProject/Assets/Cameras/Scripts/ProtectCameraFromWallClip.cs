...
{
    public class ProtectCameraFromWallClip : FL.MonoBehaviour
    {
        ...
        private FL.Transform m_Cam;                  // the transform of the camera
        private FL.Transform m_Pivot;                // the point at which the camera pivots around
        ...
        private void Start()
        {
            ...
            m_Cam = GetComponentInChildren<FL.Camera>().transform;
        ...
        private void LateUpdate()
        {
            ...
            var cols = FL.Physics.OverlapSphere(m_Ray.origin, sphereCastRadius);
            ...
            if (initialIntersect)
            {
                ...
                m_Hits = FL.Physics.RaycastAll(m_Ray, m_OriginalDist - sphereCastRadius);
            }
            else
            {
                // if there was no collision do a sphere cast to see if there were any other collisions
                m_Hits = FL.Physics.SphereCastAll(m_Ray, sphereCastRadius, m_OriginalDist + sphereCastRadius);
            }
            ...
            if (hitSomething)
            {
                FL.Debug.DrawRay(m_Ray.origin, -m_Pivot.forward*(targetDist + sphereCastRadius), Color.red);
            }
